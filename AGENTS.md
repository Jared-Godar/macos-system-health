...
- **Bounded fan-out, or do not launch.** Never start a workflow, agent fan-out,
  ...
  **Review fleets default to an agent type that does not carry the `Agent` tool** —
  ...
  what it buys; set a wall-clock kill bound in advance and honor it unasked. Never infer progress from file timestamps or
  side-channel artifacts (`find -newermt` returns nothing on macOS/BSD); if the
  harness cannot supply a countable denominator, the run is unmeasurable — do not
  start it. Prefer one well-scoped agent, or direct inspection, over parallelism.
